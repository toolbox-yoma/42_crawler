...
while index < project_size:
    ...
    print(
        "{}] {} - [{:<6}]->[{:<6}] : {}".format(
            index, result, old_id, project_id, name.text
        )
    )
    ...
    while other_index < other_project_size:
        ...
        other_attach_list = driver.find_elements(
        ...
        if update_subject:
            is_change = 1
            save_path = (
                to_path + parent_name + "/[" + str(other_index + 1) + "]" + child_name
            )
        ...
        print(
            "{}:{}] {} - [{:<6}]->[{:<6}] : {}".format(
                index, other_index, result, old_id, project_id, name.text
            )
        )
        driver.back()
        item_list = driver.find_elements(By.CLASS_NAME, "project-list-item")
